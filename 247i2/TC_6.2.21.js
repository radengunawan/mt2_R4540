...
//test case 6.2.21 Mapping Traffic from GEM Ports to Multiple U Interfaces in the Downstream Direction (1:1 VLAN Architecture)
/// Initialize Variables
var CVID1 = RandomInteger(1, 4094); /// CVID1 randomly chosen in range [1..4094]
var CVID2 = RandomIntegerExcept(1, 4094, [CVID1]); /// VID2 randomly chosen in range [1..4094]
var ONUID = 1;
var OMCC = ONUID;
...
var DnPQ1 = OMCI.GetDwPQ(OMCC, 0, 1);
var DnPQ2 = OMCI.GetDwPQ(OMCC, 1, 1);
var DnPQ3 = OMCI.GetDwPQ(OMCC, 0, 2);
...
/// Create BWMap entry for the ONU
bwmapAddEntry(ONUID, AllocId2, 20);
/// Send PLOAM Assign Alloc ID
...
/// Send config
//create GEM ports
var CTP1 = OMCI.Create(OMCC, "GEM_Port_Network_CTP", { "PortId": GEM1, "TCONTPointer": TCONT1, "TMPointerUp": UpPQ1, "PQPointerDown": DnPQ1 });
var CTP2 = OMCI.Create(OMCC, "GEM_Port_Network_CTP", { "PortId": GEM2, "TCONTPointer": TCONT2, "TMPointerUp": UpPQ2, "PQPointerDown": DnPQ2 });
var CTP3 = OMCI.Create(OMCC, "GEM_Port_Network_CTP", { "PortId": GEM3, "TCONTPointer": TCONT1, "TMPointerUp": UpPQ1, "PQPointerDown": DnPQ3 });
...
var TF1 = OMCI.Create(OMCC, "VLAN_Tagging_Filter_Data", { "meid": BPCD1, "VLANFilterList": [CVID1], "ForwardOpr": 16, "NumberOfEntries": 1 });
var TF2 = OMCI.Create(OMCC, "VLAN_Tagging_Filter_Data", { "meid": BPCD2, "VLANFilterList": [CVID2], "ForwardOpr": 16, "NumberOfEntries": 1 });
...
logInfo("Random values CVID1=" + CVID1 + " ; CVID2=" + CVID2 + "; CPbits1=" + CPbits1 + " ; CPbits2=" + CPbits2);
logInfo("Random value: GEM1=" + GEM1);
logInfo("Random value: GEM2=" + GEM2);
logInfo("Random value: GEM3=" + GEM3);

addTranslationEthToGpon(CVID1, CPbits1, GEM1);
addTranslationEthToGpon(CVID1, CPbits2, GEM2);
addTranslationEthToGpon(CVID2, CPbits1, GEM3);
...
if (TrafficGenerator.activateAutomatisation) {
    ...
    var result = TrafficGenerator.SendTemplateConfigAndDoTest("6.2.21", [
        ["<CVID1>", CVID1, 3],
        ["<CVID2>", CVID2, 3],
        ["<CPbits1>", CPbits1 << 1, 1],
        ["<CPbits2>", CPbits2 << 1, 1],
    ...
    var verdict = TrafficGenerator.ApplyVerdict(result, {
        "Frame Set A": { "Frame Set A": TrafficGenerator.nbOfPacketToSend, "Frame Set B": 0, "TID 1:1": TrafficGenerator.nbOfPacketToSend, "TID 1:2": 0 },
        "Frame Set B": { "Frame Set A": 0, "Frame Set B": TrafficGenerator.nbOfPacketToSend, "TID 2:1": TrafficGenerator.nbOfPacketToSend, "TID 2:2": 0 },
        "Frame Set C": { "Frame Set C": TrafficGenerator.nbOfPacketToSend, "TID 3:1": 0, "TID 3:2": TrafficGenerator.nbOfPacketToSend }
    });
