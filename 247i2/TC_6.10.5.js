...
//Test case 6.10.5 - Failed Software Image Download, incorrect software image CRC
...
if (lastWindowSize) {
    ...
    var dataSec = firmware.mid(nbOfWindow * windowSize * 31, firmwareSize);
    /// Send the complete section (try 3 times before aborting)
    var res_download_section = undefined;
    for (var n = 0;
        ((n < 3) && (res_download_section == undefined)); ++n) {
...
for (var i = 0;
    ...
    /// Wait 5 s
    sleep(5000);
    /// Send End of Sowftare download with bad CRC
...
if (res == undefined) testFailed("End Software Download acknowledge not received");
...
if (res != 0x01) testFailed("End Software Download response with wrong error code");
...
if (resp_get.IsValid != 0) testFailed("Image should be invalid");
