...
//Test case 6.10.3 - Failed Software Image Download, missing section
...
if (firmware.length == 0) testFailed("Unable to read the firmware file " + firmwareFilename);
...
/// Send the first window but do not send the second section
logInfo("Sending first window with missing 2nd section");
/// Get the section data block
var dataSec = firmware.mid(0, 31 * windowSize);
/// Send the complete section
sendOMCI(OMCC, "Download_Section", 0, 0, firmMeid, { "image": dataSec.toHexString(), "Skip2ndSection": 1 });
...
if (res != undefined) {
    testFailed("ONU does not refuse the window with 2nd section missing");
}
