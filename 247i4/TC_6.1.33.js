//Test Case on downstream translation for code point 3
include("Config.js");
...
OMCI.Set(OMCC, "Ext_VLAN_Tagging_Opr_Config_Data", EVTOCD1, { "InputTPID": 0x8100, "OutputTPID": 0x8100, "DownstreamMode": 3 });
...
for (var i = 0; i < 6; i++) {
    logInfo("Random values SPbit" + (i + 1) + "=" + SPbits[i]);
}
...
if (TrafficGenerator.activateAutomatisation) {
    ...
    var result = TrafficGenerator.SendTemplateConfigAndDoTest("6.1.33-1", [
        ["<SVID1>", SVIDs[0], 3],
        ["<SVID2>", SVIDs[1], 3],
        ["<SVID3>", SVIDs[2], 3],
        ["<SVID4>", SVIDs[3], 3],
    ...
    /// Create the TrafficGenerator config file from the TrafficGenerator template config file and start generation
    /// Replace SVID1 by the value formatted on 3 hex digit
    var result = TrafficGenerator.SendTemplateConfigAndDoTest("6.1.33-2", [
        ["<SVID1>", SVIDs[0], 3],
        ["<SVID2>", SVIDs[1], 3],
        ["<SVID3>", SVIDs[2], 3],
        ["<SVID4>", SVIDs[3], 3],
    ...
    /// Create the TrafficGenerator config file from the TrafficGenerator template config file and start generation
    /// Replace SVID1 by the value formatted on 3 hex digit
    var result = TrafficGenerator.SendTemplateConfigAndDoTest("6.1.33-3", [
        ["<SVID1>", SVIDs[0], 3],
        ["<SVID2>", SVIDs[1], 3],
        ["<SVID3>", SVIDs[2], 3],
        ["<SVID4>", SVIDs[3], 3],
