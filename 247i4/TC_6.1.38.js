//Test Case on downstream translation for code point 8
include("Config.js");
...
OMCI.Set(OMCC, "Ext_VLAN_Tagging_Opr_Config_Data", EVTOCD1, { "InputTPID": 0x8100, "OutputTPID": 0x8100, "DownstreamMode": 8 });
...
if (TrafficGenerator.activateAutomatisation) {
    ...
    var result = TrafficGenerator.SendTemplateConfigAndDoTest("6.1.38-1", [
        ["<SVID1>", SVIDs[0], 3],
        ["<SVID2>", SVIDs[1], 3],
        ["<SVID3>", SVIDs[2], 3],
        ["<SVID4>", SVIDs[3], 3],
    ...
    var local_verdict = TrafficGenerator.ApplyVerdict(result, {
        ...
        "Frame Set Ads": { "Frame Set Ads": 0, "TID 5:1": 0 },
        "Frame Set Bds": { "Frame Set Bds": 0, "TID 6:1": 0 },
        "Frame Set Cds": { "Frame Set Cds": 0, "TID 7:1": 0 },
        "Frame Set Dds": { "Frame Set Dds": 0, "TID 8:1": 0 }
    });
    ...
    /// Create the TrafficGenerator config file from the TrafficGenerator template config file and start generation
    /// Replace SVID1 by the value formatted on 3 hex digit
    var result = TrafficGenerator.SendTemplateConfigAndDoTest("6.1.38-2", [
        ["<SVID1>", SVIDs[0], 3],
        ["<SVID2>", SVIDs[1], 3],
        ["<SVID3>", SVIDs[2], 3],
        ["<SVID4>", SVIDs[3], 3],
    ...
    var local_verdict = TrafficGenerator.ApplyVerdict(result, {
        ...
        "Frame Set Eds": { "All Ds": 0, "TID 5:1": 0 },
        "Frame Set Fds": { "All Ds": 0, "TID 6:1": 0 },
        "Frame Set Gds": { "All Ds": 0, "TID 7:1": 0 },
        "Frame Set Hds": { "All Ds": 0, "TID 8:1": 0 }
    });
    TrafficGenerator.Disconnect();
    if (local_verdict == false) logError("Error on received stream");
    verdict = local_verdict && verdict;
    ...
    /// Create the TrafficGenerator config file from the TrafficGenerator template config file and start generation
    /// Replace SVID1 by the value formatted on 3 hex digit
    var result = TrafficGenerator.SendTemplateConfigAndDoTest("6.1.38-3", [
        ["<SVID1>", SVIDs[0], 3],
        ["<SVID2>", SVIDs[1], 3],
        ["<SVID3>", SVIDs[2], 3],
        ["<SVID4>", SVIDs[3], 3],
