//Create and Remove Multicast Groups in the Dynamic Access Control List table
testPassed("Create and Remove Multicast Groups in the Dynamic Access Control List table");
...
if (TrafficGenerator.activateAutomatisation) {
    ...
    var IP_S1 = IPv4Address("128.0.0.2");
    var IP_S2 = IPv4Address("128.0.0.3");
...
OMCI.Set(OMCC, "Multicast_Opr_Profile", MOP1, { 'UnauthorizedJoinReqBehaviour': 0 });

var MSCI1 = OMCI.Create(OMCC, "Multicast_Subsc_Config_Info", { "meid": BPCD3, "MulticastOprProfilePtr": MOP1 });

testPassed("Multicast Subscription Monitor created (step 8)");
...
if (TrafficGenerator.activateAutomatisation) {
    ...
    var struct = TrafficGenerator.SendTemplateConfig("6.3.17", [
        ["<SPbit1>", SPbit1 << 1, 1],
        ["<SVID1>", SVID1, 3],
    ...
    if (0 == popup("Are Multicast flow 1 or 2 received at U interface?", "YesNo"))
        testFailed("Multicast Channel received at U interface when they must be discarded");
}
...
///Step 7. Cause the OLT Emulator to add IP-G1 to the dynamic access list control table of Multicast Operations Profile ME at the ONU

logInfo("#############################");
testPassed("Step 7. Cause the OLT Emulator to add IP-G1 to the dynamic access list control table of Multicast Operations Profile ME at the ONU");
...
///Step 8. At the U-interface send IGMP message to join channel Ch1       (CH1 is allowed)

logInfo("#############################");
testPassed("Step 8. At the U-interface send IGMP message to join channel Ch1");
if (TrafficGenerator.activateAutomatisation) {
    ...
    popup("Traffic Configuration", "Send a Join for Channel 1 and wait till it has taken effecr, then start sending downstream flows 1 & 2");
...
///Step 9. Cause the OLT Emulator to add IP-G2 to the dynamic access list control table of Multicast Operations Profile ME at the ONU

logInfo("#############################");
testPassed("Step 9. Cause the OLT Emulator to add IP-G2 to the dynamic access list control table of Multicast Operations Profile ME at the ONU");
...
if (TrafficGenerator.activateAutomatisation) {
    ...
    popup("Traffic Configuration", "Start sending downstream flows 1 & 2");
    if (0 == popup("Question", "Is Multicast flow 1 received at U interface?", "YesNo"))
        testFailed("Multicast Channel 1 not received at U interface");
    if (0 != popup("Question", "Is Multicast flow 2 received at U interface?", "YesNo"))
        testFailed("Multicast Channel 2 received at U interface");
...
///Step 10. At the U-interface send IGMP message to join channel Ch2       (CH1 and CH2 are allowed)

logInfo("#############################");
testPassed("Step 10. At the U-interface send IGMP message to join channel Ch2");
...
if (TrafficGenerator.activateAutomatisation) {
    ...
    popup("Traffic Configuration", "Send a Join for Channel 2 and wait till it has taken effecr, then start sending downstream flows 1 & 2");
    if (0 == popup("Question", "Is Multicast flow 1 received at U interface?", "YesNo"))
        testFailed("Multicast Channel 1 not received at U interface");
    if (0 == popup("Question", "Is Multicast flow 2 received at U interface?", "YesNo"))
...
///Step 11. Sending Leave channel 1& 2 message on U-interface

logInfo("#############################");
testPassed("Step 11. Sending Leave channel 1& 2 message on U-interface");
...
///Step 12. Cause the OLT emulator to remove Multicast Operations Profile ME entry IND1

logInfo("#############################");
testPassed("Step 12. Cause the OLT emulator to remove Multicast Operations Profile ME entry IND1");
OMCI.Set(OMCC, "Multicast_Opr_Profile", MOP1, {
    "DynamicACLTable": {
        "TableIndex": 0x8000
    }
...
///Step 13. At the U-interface send IGMP messages to join channel Ch1 and Ch2       (CH1 is NOT allowed anymore / CH2 is still allowed)

logInfo("#############################");
testPassed("Step 13. At the U-interface send IGMP messages to join channel Ch1 and Ch2");
if (TrafficGenerator.activateAutomatisation) {

    var localVerdict = TrafficGenerator.TestSingleUpstreamFlow(1, 2, upstreamFlows, 0, true);
    verdict = localVerdict && verdict;
    var localVerdict = TrafficGenerator.TestSingleUpstreamFlow(3, 2, upstreamFlows, 1);
    verdict = localVerdict && verdict;
