//Test Case on downstream translation for code point 1
include("Config.js");
...
OMCI.Set(OMCC, "Ext_VLAN_Tagging_Opr_Config_Data", EVTOCD1, { "InputTPID": 0x8100, "OutputTPID": 0x8100, "DownstreamMode": 1 });
...
if (TrafficGenerator.activateAutomatisation) {
    ...
    ///Replication rules: downstream (note: max 8 rules at a time)
    ...
    addTranslationGponToEthExt(1, 0xffff, 0xff, 0xffff, SVIDs[0], SPbits[0], 0x8100, 0, GEM1);
    addTranslationGponToEthExt(1, 0xffff, 0xff, 0xffff, SVIDs[0], SPbits[1], 0x8100, 0, GEM2);
    addTranslationGponToEthExt(1, 0xffff, 0xff, 0xffff, SVIDs[1], SPbits[2], 0x8100, 0, GEM3);
    addTranslationGponToEthExt(1, 0xffff, 0xff, 0xffff, SVIDs[2], SPbits[3], 0x8100, 0, GEM4);
    ...
    var result = TrafficGenerator.SendTemplateConfigAndDoTest("6.1.31-1", [
        ["<SVID1>", SVIDs[0], 3],
        ["<SVID2>", SVIDs[1], 3],
        ["<SVID3>", SVIDs[2], 3],
        ["<SVID4>", SVIDs[3], 3],
    ...
    addTranslationEthToGpon(SVIDs[3], SPbits[4], GEM4); ///Flow G ds
    addTranslationEthToGpon(SVIDs[1], SPbits[0], GEM1); ///Flow H ds

    ///Replication rules: upstream (note: max 8 rules at a time)
    addTranslationGponToEthExt(1, 0xffff, 0xff, 0xffff, SVIDs[2], SPbits[2], 0x8100, 0, GEM3);
    ...
    /// Create the TrafficGenerator config file from the TrafficGenerator template config file and start generation
    /// Replace SVID1 by the value formatted on 3 hex digit
    var result = TrafficGenerator.SendTemplateConfigAndDoTest("6.1.31-2", [
        ["<SVID1>", SVIDs[0], 3],
        ["<SVID2>", SVIDs[1], 3],
        ["<SVID3>", SVIDs[2], 3],
        ["<SVID4>", SVIDs[3], 3],
    ...
    /// Check verdict for all streams
    local_verdict = TrafficGenerator.ApplyVerdict(result, {
    ...
    /// Create the TrafficGenerator config file from the TrafficGenerator template config file and start generation
    /// Replace SVID1 by the value formatted on 3 hex digit
    var result = TrafficGenerator.SendTemplateConfigAndDoTest("6.1.31-3", [
        ["<SVID1>", SVIDs[0], 3],
        ["<SVID2>", SVIDs[1], 3],
        ["<SVID3>", SVIDs[2], 3],
        ["<SVID4>", SVIDs[3], 3],
    ...
    /// Check verdict for all streams
    local_verdict = TrafficGenerator.ApplyVerdict(result, {
        "Frame Set Ids": { "Frame Set Ids": TrafficGenerator.nbOfPacketToSend, "TID 5:1": TrafficGenerator.nbOfPacketToSend },
    });
    verdict = local_verdict && verdict;
    ...
    addTranslationEthToGpon(0xffff, 0xff, GEM4);
