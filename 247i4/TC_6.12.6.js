//Create, Delete and Add New Services
testPassed("Create, Delete and Add New Services");
include("Config.js");
...
if (TrafficGenerator.activateAutomatisation) {
    ...
    var result = TrafficGenerator.SendTemplateConfigAndDoTest("6.12.6-1-us", [
        ["<SPbit1>", SPbit1 << 1, 1],
        ["<SVID1>", SVID1, 3],
        ["<CPbit1>", CPbit1 << 1, 1],
        ["<CVID1>", CVID1, 3],
    ...
    var result = TrafficGenerator.SendTemplateConfigAndDoTest("6.12.6-1-ds", [
        ["<SPbit1>", SPbit1 << 1, 1],
        ["<SVID1>", SVID1, 3],
        ["<CPbit1>", CPbit1 << 1, 1],
        ["<CVID1>", CVID1, 3],
    ...
    popup("Please run flows A, B, C, D, E, F");
    if (0 == popup("Are flows A, B, C, D received properly ?", "YesNo"))
        testFailed("Configured Flow are not all received properly");
    if (0 == popup("Are flows E & F discarded properly ?", "YesNo"))
        testFailed("Un configured Flows are not all discarded");

}

logInfo("################################");
logInfo("################################");
testPassed("Delete Service -- Step 8");
...
logInfo("################################");
testPassed("Testing configuration between 1&2 -- Step 9 & 10");
...
if (TrafficGenerator.activateAutomatisation) {
    ...
    popup("Please run flows A, B, C, D, E, F");
    if (0 == popup("Are flows A & C received properly ?", "YesNo"))
        testFailed("Configured Flow are not all received properly");
    if (0 == popup("Are flows B, D, E & F discarded properly ?", "YesNo"))
        testFailed("Un configured Flows are not all discarded");
...
logInfo("################################");
logInfo("################################");
testPassed("Create new Service -- Step 11");
...
logInfo("################################");
testPassed("Testing configuration 2 -- Step 12 & 13");
...
if (TrafficGenerator.activateAutomatisation) {
    ...
    popup("Please run flows A, B, C, D, E, F");
    if (0 == popup("Are flows A, E, C, F received properly ?", "YesNo"))
        testFailed("Configured Flow are not all received properly");
    if (0 == popup("Are flows B & D discarded properly ?", "YesNo"))
        testFailed("Un configured Flows are not all discarded");
}
