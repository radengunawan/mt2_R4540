//Test Case on OMCI reboot without OMCI configuration persistence
testPassed("Test Case on OMCI reboot without OMCI configuration persistence");
include("Config.js");
...
if (TrafficGenerator.activateAutomatisation) {
    ...
    var result = TrafficGenerator.SendTemplateConfigAndDoTest("6.9.7", [
        ["<SVID1>", SVID1, 3],
        ["<CVID1>", CVID1, 3],
        ["<CPbit1>", CPbit1 << 1, 1],
        ["<SPbit1>", SPbit1 << 1, 1],
...
if (dataSyncAfterReboot == dataSyncBeforeReboot)
    testFailed("This test is  not applicable to this ONU because MIB data sync reports a non-zero value after the ONU reboot command");

if (dataSyncAfterReboot != 0)
    testFailed("Pass/Fail 6 At Step 7, The MIB data sync OMCI message reports a non 0 value");
