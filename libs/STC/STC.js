...
STC = {
    ...
    ///parasiteTrafficPacketThreshold -- if there is too much parasite traffic, fail the test
    parasiteTrafficPacketThreshold: 15,
    ...
    StartAnalyzerOnPort: function(portIndex) {
        this.StcStartAnalyzer(this.ports[portIndex].analyzer);
    },

    StartAnalyzers: function() {
        /// Start all analyzer
        for (var analysisPort in this.ports) {
            this.StcStartAnalyzer(this.ports[analysisPort].analyzer);
        }
    },
    ...
    StopAnalyzerOnPort: function(portIndex) {
        this.StcStopAnalyzer(this.ports[portIndex].analyzer);
    },

    StopAnalyzers: function() {
        /// Stop all analyzer
        for (var analysisPort in this.ports) {
            this.StcStopAnalyzer(this.ports[analysisPort].analyzer);
        }
    },
            ...
            if (rule == undefined)
                ...
                for (var r in rule) {
                    if (result[stream][r] == undefined) { ///Add again when TID is working
                        ...
                        var ruleVerdict = true;
                        if (result[stream][r] == rule[r]) {
                            TrafficGenerator.logInfo("Filter " + r + " has correctly received " + result[stream][r] + " packet(s)");
                        } else {
                            if (rule[r] > 0) {
                                ///Forwarding rule
                                if (result[stream][r] < (rule[r] - TrafficGenerator.toleranceInFrame)) {
                                    ruleVerdict = false;
                                }
                            } else {
                                ///Discarding rule
                                if (result[stream][r] > this.parasiteTrafficPacketThreshold) {
                                    ruleVerdict = false;
                                }
                            }
                            if (ruleVerdict) {
