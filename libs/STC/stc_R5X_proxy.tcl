...
proc new_stream { port frame_len frame_config } {
  ...
  set hFrame1 [stc::create streamBlock -under $port -enableHighSpeedResultAnalysis FALSE -insertSig true -frameConfig "" -frameLengthMode FIXED -maxFrameLength $frame_len -FixedFrameLength $frame_len]
...
proc subscribe_to_filters { project port_rx } {
  ...
  stc::subscribe -Parent $project \
                ...
                -viewAttributeList "framecount sigframecount Comp32" \
                -interval 1
  return "OK"
}
...
proc get_filter_stat { port } {
  ...
  foreach hResults [stc::get $port.Analyzer -children-FilteredStreamResults] {
    ...
    set szResults $aResults(-SigFrameCount)
...
proc get_filter_stat_idx { port stream } {
  ...
  set hStreamResults [stc::get $stream -children-txstreamresults]
  set stream_id  [stc::get $hStreamResults -StreamId]
  puts "Stream Id of $stream is $stream_id"
  ...
  foreach hResults [stc::get $port.Analyzer -children-FilteredStreamResults] {
    ...
    #   were tracked.
	puts "current Stream Id=$aResults(-Comp32)"
	puts "Sig Frame Cnt=$aResults(-SigFrameCount)"
	set idx $aResults(-Comp32)
	if { $idx == $stream_id } {
		puts "Found"
		set value $aResults(-SigFrameCount)
